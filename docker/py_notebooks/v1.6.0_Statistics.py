...
# %%
# %%time
# Build SPEECH_INDEX using the new pyriksdagen pipeline (~3.5 min for full corpus)
...
cfg = CorpusConfig()
loader = CorpusLoader(config=cfg)
loader.initialize(threads=26, force_update=False)

ml = MetadataLoader(cfg)
ml.initialize()
SPEECH_INDEX = ml.enrich_speech_dataframe(loader.speech_dataframe)
print(f'Built SPEECH_INDEX: {len(SPEECH_INDEX):,} speeches')

# %%
# Load metadata CSVs for downstream analysis (overlaps, ministers, MP checks)
# Old MetadataParser doesn't handle new CSV format, so load directly
metadata_dir = 'riksdagen-corpus/persons/data/'
metadata = type('MetadataCompat', (), {'metadata': {}})()
for csv_name in ['person', 'minister', 'member_of_parliament', 'party_affiliation',
                 'party_abbreviation', 'name']:
    metadata.metadata[csv_name] = pd.read_csv(os.path.join(metadata_dir, f'{csv_name}.csv'))
    if 'person_id' in metadata.metadata[csv_name].columns:
        metadata.metadata[csv_name] = metadata.metadata[csv_name].rename(
            columns={'person_id': 'swerik_id'})

# Prepare party_affiliation date columns (used by overlap analysis cells)
aff = metadata.metadata['party_affiliation']
aff['start_dt'] = pd.to_datetime(aff['start'], errors='coerce')
aff['end_dt'] = pd.to_datetime(aff['end'], errors='coerce')
metadata.metadata['party_affiliation'] = aff

# Compatibility methods that downstream cells call
...
def get_affiliation_from_dates(who, dates):
    aff = metadata.metadata['party_affiliation']
    if who == "unknown":
        return [("unknown", False, "Unknown person") for _ in dates]
    df = aff[aff['swerik_id'] == who]
    if len(df) == 0:
        return [("unknown_missing", False, "No listing") for _ in dates]
    if len(df) == 1:
        return [(df.party.values[0], False, "Only one match") for _ in dates]
    n_unique = df.party.nunique()
    results = []
    for date in dates:
        ...
        if len(result) == 1:
            ...
            results.append((df.party.values[0], False, "Only one unique party"))
        elif len(result) == 0:
            results.append((df.party.values[0], True, f"Estimated ({n_unique} parties)"))
        else:
            results.append((result.party.values[-1], True, "Multiple matches"))
    return results

def find_words_in_metadata(words):
    words_lower = [w.lower() for w in words]
    dfs = {}
    for name, df in metadata.metadata.items():
        mask = df.apply(lambda row: all(w in " ".join(str(i) for i in row).lower()
                                        for w in words_lower), axis=1)
        if mask.any():
            dfs[name] = df[mask]
    return dfs

metadata.join_on = join_on
metadata.convert_date = convert_date
metadata.get_affiliation_from_dates = get_affiliation_from_dates
metadata.find_words_in_metadata = find_words_in_metadata

# %%
MD('Sanity check to ensure the material has been loaded.')
df = metadata.metadata['person']

df[df.swerik_id == 'i-14itFAMq9exF2LvnaCtHAn']

# %%
# SPEECH_INDEX already has party_affiliation, party_abbrev, name, gender, year, born, dead from the pipeline
# (Old cells 106-117 did joins; now pre-computed by MetadataLoader.enrich_speech_dataframe)
# born/dead are already present; no separate join needed

df = SPEECH_INDEX.copy()
df.loc[df[df.gender.isna()].index,'gender'] = 'unknown'
df.loc[df[df.party_abbrev.isna()].index, 'party_abbrev'] = '?'
SPEECH_INDEX = df
...
loader.empty_speeches[['file_name','protocol','who_intro_id','who_intro']].to_csv(f'output/{corpus_version_string}/empty_speeches.csv',sep='~')
loader.empty_speeches.protocol.value_counts().head(5)
...
members_who_have_party_defined_at_least_once = df[~df.party.isna()].swerik_id.unique()
# Strip people who have a defined party at least once!
df = df[~df.swerik_id.isin(members_who_have_party_defined_at_least_once)]
...
df2 = df[(df.start_year >= 1920) & (df.end_year >= 1920)].groupby('swerik_id').party.agg(list).apply(np.unique).to_frame()
...
def remove_duplicates(df):
    ...
    return df
complete_df = remove_duplicates(df[df.swerik_id.isin(unknown_ids)].sort_values(by='swerik_id').groupby('swerik_id').agg(list))
...
MD(f'# Number of members of parliament missing either start or end date: {len(df[(df.end.isna() | df.start.isna())].swerik_id.unique())}')
df = df[~df.end.isna()]

df.end = df.end.apply(metadata.convert_date)
members_ending_after_1920 = df[df.end.apply(lambda x: x.year) >= 1920]
display(members_ending_after_1920)
who_ending_after_1920 = members_ending_after_1920.swerik_id.unique()
...
fig = px.bar(df.groupby('gender').count()[['swerik_id', 'dead', 'riksdagen_id']],
       height=600,
       barmode='group',
       labels={'gender':'Kön','value':'Antal','variable':'Datapunkt'},
            title='Översikt')
# print(fig)
translate = {'swerik_id':'Totalt antal', 'dead':'Avlidna', 'riksdagen_id':'Antal i riksdagen'}
...
df2 = pd.DataFrame(df.groupby(['role','gender']).nunique()['swerik_id_left'])
...
df = df.merge(metadata.metadata['person'], on='swerik_id')
df2 = pd.DataFrame(df.groupby(['role','gender']).nunique()['swerik_id'])
df2 = df2.unstack(level=1)
df2.columns = [y for x, y in df2.columns.to_flat_index()]
# px.bar(df2,barmode='group',height=600)
...
df = metadata.join_on(df, pd.DataFrame(df.groupby('swerik_id').party.count()).reset_index().rename(columns={'party':'party_count'}), column='swerik_id')
ids = []
for name, group in df[(df.party_count >= 2)].groupby('swerik_id'):
    if len(group.start_dt.unique()) == 1 and len(group.end_dt.unique()) == 1:
        ids.extend(group.swerik_id.unique())
...
df2 = df
df2['overlap'] = (df2.groupby('swerik_id')
...
total_dataframes = []
for id, count in df2[df2.overlap == True].swerik_id.value_counts().items():
    tmp = df2[df2.swerik_id == id]
    if tmp.party.nunique() == 1:
        continue
    # if not different_party_overlap(tmp):
    #     continue
    total_dataframes.append(df2[df2.swerik_id == id])
    # display(df2[df2.swerik_id == id])
...
overlapping_ids = set(overlap_affiliations.swerik_id.values)
...
# df = metadata.metadata['party_affiliation']
all_overlap = []
for id, group in tqdm(df.groupby('swerik_id')):
...
display(df2[df2.swerik_id == 'i-HKCBHNaesqPFHjWMxwcBN4'])
df3 = metadata.metadata['party_affiliation']
display(df3[df3.swerik_id == 'i-HKCBHNaesqPFHjWMxwcBN4'])
df4 = df3[df3.swerik_id == 'i-HKCBHNaesqPFHjWMxwcBN4'][['party','start_dt','end_dt']]
...
check_values(metadata.metadata['party_affiliation'], 'swerik_id', filtered_df.swerik_id.values).party.value_counts()
