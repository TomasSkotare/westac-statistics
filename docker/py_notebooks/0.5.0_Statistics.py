...
parser = corpus_parser.CorpusParser('./riksdagen-corpus/corpus/protocols/', 'corpus_db_0.5.0.db')
...
# %%
metadata.metadata['member_of_parliament'].role.unique()
...
df = pd.DataFrame(df)
df.columns = ['count']
df.to_csv('output/known_person_but_unknown_party.csv',encoding='utf-8-sig', sep=';')
display(df.head(10))
...
df.to_csv('output/most_common_unknown_introductions_with_files.csv',encoding='utf-8-sig', sep=';')
df.head(30)
...
for _min, _max in token_ranges:
    ...
    data.index = [year for tokens, year in data.index]
    with pd.ExcelWriter('./output/TTR_Over_Time.xlsx', engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:
...
for _min, _max in token_ranges:
    ...
    for decade in sorted(range_data.decade.unique()):
        ...
        with pd.ExcelWriter(f'./output/TTR_Top_{top_bottom_count}_({_min}-{_max}_tokens).xlsx', engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:
        ...
        with pd.ExcelWriter(f'./output/TTR_Bottom_{top_bottom_count}_({_min}-{_max}_tokens).xlsx', engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:            
...
# data.index = [year for tokens, year in data.index]
with pd.ExcelWriter('./output/Speech_count_over_time.xlsx', engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:
    data.to_excel(writer,sheet_name='All speeches')
data.to_excel('./output/Speech_count_over_time.xlsx',sheet_name='All speeches')
...
plot = px.line(data,title=f'./output/Speech count over time, only speeches with > {MINIMUM_TOKENS} tokens', labels={'index':'Year','value':'count'},height=600)
plot.update_layout(hovermode='x')
...
# %%
SPEECH_INDEX.groupby(['year','party_abbrev']).n_tokens.mean().unstack().to_excel('./output/Average tokens per year per party.xlsx')
SPEECH_INDEX.groupby(['year','gender']).n_tokens.mean().unstack().to_excel('./output/Average tokens per year per gender.xlsx')
SPEECH_INDEX.groupby(['year']).n_tokens.mean().to_excel('./output/Average tokens per year per person.xlsx')

# %%
# Speeches over time
SPEECH_INDEX.groupby(['year']).u_id.count().to_excel('./output/Speeches per year over time.xlsx')
...
# %%
SPEECH_INDEX[SPEECH_INDEX.who.isin(undefined_timespan_ids)].groupby('year').who.value_counts().unstack().to_excel('./output/Undefined party by year.xlsx')
...
# %%
SPEECH_INDEX.to_feather('speech_index_0.4.6_dev.feather')
...
for txt in tqdm(df.text.values):
    n_grams = ngrams(tokenizer.tokenize(" ".join(txt).lower()), n)
    all_ngrams.extend(n_grams)
# for grams in n_grams:
#     print(grams)

df2 = pd.DataFrame(all_ngrams)
df2.groupby([0,1,2]).agg(len)
