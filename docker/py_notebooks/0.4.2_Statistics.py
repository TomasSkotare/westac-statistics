...
con = sqlite3.connect('./tagged_frames_v0.4.2_speeches.beta.feather/riksprot_metadata.v0.4.2.db')
cursor = con.cursor()
...
SPEECH_INDEX = pd.read_feather('./tagged_frames_v0.4.2_speeches.beta.feather/document_index.feather')
...
# %%
px.line(SPEECH_INDEX.groupby('year').agg(**{'Number of speakers':('who','nunique')}))
...
# df = 
px.bar(df[df.party.map(lambda x: x is None)].groupby(['year']).size
...
df['UnknownPart'] = df.Unknown / (df.Known + df.Unknown)
display(px.bar(df, y='UnknownPart', title='Parts of tokens spoken by unknown speakers per year'))
px.line(df.UnknownPart.map(lambda x: (1-x) * 100).rename('Known part'), labels={'value':'Percent known'})

# %%

# %%
df = SPEECH_INDEX.set_index('speaker_hash').join(all_tables['_unknowns'].set_index('hash'), rsuffix='_r')
# df[df.party=='unknown'].reset_index().rename(columns={'index':'hash'}).party_r.unique()
...
# %%
df = all_tables['unknown_utterance_party']
df
# df[df.u_id == 'i-84a8d0d09d6b917c-1']

# %%
df = all_tables['_party_abbreviation'].party.str.lower().unique()
sorted(df)
...
year_group = SPEECH_INDEX.drop(columns='document_id').groupby('year')year_group = SPEECH_INDEX.drop(columns='document_id').groupby('year')
...
# %%
df = SPEECH_INDEX[SPEECH_INDEX.who != 'unknown']
unknown_people_ids = df[df.party == 'unknown'].who.unique()
df = all_tables['_person']
# df
join_on(df[df.person_id.isin(unknown_people_ids)], all_tables['_name'], column='person_id')
...
with pd.option_context('display.max_rows', None, 'display.max_columns', None):  # more options can be specified also
    display(df.sort_values(by='year_of_birth')) # REMOVE HEAD HERE IF YOU WANT ALL
