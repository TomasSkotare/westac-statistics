...
con = sqlite3.connect('./tagged_frames_v0.4.3_speeches.feather/riksprot_metadata.v0.4.3.db')
cursor = con.cursor()
...
# %%
__all_feather_files = glob('./tagged_frames_v0.4.3_speeches.feather/**/*.feather',recursive=True)
__all_feather_files_dict = {os.path.basename(x):x for x in all_files}
...
with pd.option_context('display.max_rows', None, 'display.max_columns', None):  # more options can be specified also
    display(df[df['Number of unique speakers'] == 1].set_index('document_name')['Number of unique speakers'])
...
# %% [markdown]
# # Check if this means anything (I.e. apk is marked as "stadsminister")

# %%
df = SPEECH_INDEX.set_index('speaker_note_id').join(all_tables['_unknowns'].set_index('uuid'), rsuffix='_r')
# df[df.party=='unknown'].reset_index().rename(columns={'index':'hash'}).party_r.unique()
pd.set_option("display.max_columns", None)
df[df.party_r == '(apk)']

# %%
all_tables['_unknowns']
