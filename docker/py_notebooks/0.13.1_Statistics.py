...
df = metadata.join_on(df, metadata.metadata['name'].rename(columns={'swerik_id':'who'}), column='who')
df = metadata.join_on(df, metadata.metadata['person'].rename(columns={'swerik_id':'who'}), column='who')
...
def measure_io(df, format):
    # Define file path
    file_path = f'/tmp/tempfile.{format}'

    # Measure write time
    start_time = time.time()
    if format == 'feather':
        df.to_feather(file_path, compression='lz4', compression_level=9)
    elif format == 'parquet':
        df.to_parquet(file_path)
    else:
        raise ValueError("Invalid format. Choose either 'feather' or 'parquet'.")
...
def compare_formats(df):
    for format in ['feather', 'parquet']:
        write_time, read_time, file_size = measure_io(df, format)
