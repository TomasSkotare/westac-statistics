...
parser = corpus_parser.CorpusParser('./riksdagen-corpus/corpus/protocols/', 'corpus_db.db')
...
df = pd.read_excel('Partier och förkortningar.xlsx',header=None).drop_duplicates()
df.columns=['party','abbreviation', 'Comment']
df.abbreviation = df.abbreviation.str.capitalize()
metadata.metadata['party_abbreviation'] = df
metadata.add_affiliation(parser.speech_dataframe)

df = metadata.join_on(parser.speech_dataframe, metadata.metadata['party_abbreviation'].rename(columns={'party':'party_affiliation'}).drop(columns='Comment'),column='party_affiliation') 
df.abbreviation = df.abbreviation.str.capitalize()
...
# %%
df = metadata.metadata['person'].groupby('wiki_id').agg(list)
df['count'] = df.dead.apply(lambda x: len(x))
df.sort_values(by='count',ascending=False)

# %% [markdown]
# # Known people, who have no party in party_affiliation.csv, but has a party in member_of_parliament.csv
...
df = pd.DataFrame(df)
df.columns = ['count']
df.to_csv('known_person_but_unknown_party.csv',encoding='utf-8-sig', sep=';')
display(df.head(10))
...
# %%
SPEECH_INDEX.party_affiliation.value_counts()

# %% [markdown]
# # Most common speaker introductions (in decending order)

# %%
df = pd.DataFrame(SPEECH_INDEX[(SPEECH_INDEX.who == 'unknown')].who_intro.value_counts().sort_values(ascending=False))
df.to_csv('most_common_unknown_introductions.csv',encoding='utf-8-sig', sep=';')
df.head(30)
...
# ## Currently not included in the data, consider if it should be included again
...
#     **{'Number of speakers':('who','nunique'),'Number of speeches (total)':('year','count'), 'Number of tokens (total)':('n_tokens','sum')})
# data['Tokens per member and year'] = np.round(data['Number of tokens (total)'] / data['Number of speakers'],2)
# data.to_excel('tokens_per_year_grouped_by_gender_and_party.xlsx')
data = SPEECH_INDEX.groupby(['year','party_abbrev']).agg(
    **{'Number of speakers':('who','nunique'),'Number of speeches (total)':('year','count'), 'Number of tokens (total)':('n_tokens','sum')})
data['Tokens per member and year'] = np.round(data['Number of tokens (total)'] / data['Number of speakers'],2)
data.to_excel('tokens_per_year_grouped_by_party.xlsx')
...
display(SPEECH_INDEX[SPEECH_INDEX.n_tokens > 10000])
...
df['count'] = df.party_affiliation.apply(lambda x: len(x))
df.sort_values(by='count', ascending=False)
...
def try_count(u_id):
    try:
        ...
        return (u_id, len(count) / np.sum(list(count.values())))
    except:
        return (u_id,np.nan)

def get_ttr_from_uid(u_ids):
    with Pool(26) as p:
        return p.map(try_count, u_ids)
...
if PERFORM_RECALCULATION:
    ...
    df.to_feather('uid_ttr.feather')
else:
    df = pd.read_feather('uid_ttr.feather')
...
for _min, _max in token_ranges:
    ...
    data.index = [year for tokens, year in data.index]
    with pd.ExcelWriter('TTR_Over_Time.xlsx', engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:
...
for _min, _max in token_ranges:
    ...
    for decade in sorted(range_data.decade.unique()):
        ...
        with pd.ExcelWriter(f'TTR_Top_{top_bottom_count}_({_min}-{_max}_tokens).xlsx', engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:
        ...
        with pd.ExcelWriter(f'TTR_Bottom_{top_bottom_count}_({_min}-{_max}_tokens).xlsx', engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:            
...
# data.index = [year for tokens, year in data.index]
with pd.ExcelWriter('Speech_count_over_time.xlsx', engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:
    data.to_excel(writer,sheet_name='All speeches')
data.to_excel('Speech_count_over_time.xlsx',sheet_name='All speeches')
...
# data.to_excel('Speech_count_over_time.xlsx',sheet_name=f'Speeches longer than {MINIMUM_TOKENS} tokens')
...
# %%
SPEECH_INDEX.groupby(['year','party_abbrev']).n_tokens.mean().unstack().to_excel('Average tokens per year per party.xlsx')
SPEECH_INDEX.groupby(['year','gender']).n_tokens.mean().unstack().to_excel('Average tokens per year per gender.xlsx')
SPEECH_INDEX.groupby(['year']).n_tokens.mean().to_excel('Average tokens per year per person.xlsx')

# %%
# Speeches over time
SPEECH_INDEX.groupby(['year']).u_id.count().to_excel('Speeches per year over time.xlsx')
...
# %%
SPEECH_INDEX[SPEECH_INDEX.who.isin(undefined_timespan_ids)].groupby('year').who.value_counts().unstack().to_excel('Undefined party by year.xlsx')
...
df.drop(columns='average_decade').to_excel('uncertain_per_decade.xlsx')
...
# token_ranges = [(x[0], x[-1]) for x in np.array_split(range(1, df.n_tokens.max()),100)]


df['token_range'] = df.n_tokens.apply(lambda x: next(iter([f'{a}-{b}' for a,b in token_ranges if ((x >= a) & (x <= b))]),'None'))


# df[['u_id','who','decade','party_abbrev','ttr']].to_excel('ttr_short.xlsx')

idxes = []
for name, group in df.groupby(['date','token_range']):
    idxes.extend([group.ttr.idxmax(), group.ttr.idxmin()])
df = df.loc[idxes]    
fig, ax = plt.subplots(figsize=(20, 20))
sns.scatterplot(data=df, y='ttr', x='n_tokens', ax=ax, hue='decade', s=1)
...
# # %%time
# df = SPEECH_INDEX
# df['speeches_this_year'] = df.groupby(['who','year'])['party_abbrev'].transform(len)
# df
...
# %%
# %%time
df['hash_repeats'] = df.groupby('hash')['hash'].transform(len)
